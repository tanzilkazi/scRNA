...
set.seed(3)
n_groups <- 2
params <- newSplatParams(batchCells=n_groups*1000, group.prob = rep(1/n_groups, time=n_groups))
params <- setParams(params, de.prob=1,nGenes=2)
groups <- splatSimulate(params, method ="groups", verbose = FALSE)
dim(counts(groups))

graph_data <- log2(counts(groups)+1)
graph_data <-data.frame(t(graph_data))
pl <- ggplot(graph_data,aes(x=graph_data$Gene1, y=graph_data$Gene2)) + geom_point(aes(col=groups$Group))
pl
